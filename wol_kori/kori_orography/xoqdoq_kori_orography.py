...
from kori_input import winspeed, kori_input, sumsum
from function import calc_sigma, calc_d, calc_he, u_cor, calc_Et, opentr
from kori_orography import index, orography
...
####################################################
x = np.arange(1,2000,90) # x: distance in meters
...
for k in range(len(zeniths)):
    for l in range(len(azimuths)-1):
        ...
        if l <= 7:
            values[l+8,k] = xoq[k,l] ## select xoq or doq
        elif l > 7:
            values[l-8,k] = xoq[k,l] ## select xoq or doq
...
lev = [c for c in np.arange(0,maxoq*1.1,maxoq/10, dtype=float)]
